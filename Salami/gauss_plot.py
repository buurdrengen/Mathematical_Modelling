...
surf = ax.plot_surface(x1,x2,g, cmap = "gist_ncar", linewidth=0, rstride=1, cstride=1)
plt.show()
...
g_cors = g_cor(rho=rho)

surf = ax.plot_surface(x1,x2,g_cors, cmap = "gist_ncar", linewidth=0, antialiased=False, rstride=1, cstride=1)
...
ax.set_zlim([0,0.03])
fig.colorbar(surf, shrink=0.7, aspect=20, pad=0.10, ticks = np.arange(0,0.035,0.005))
...
with movie.saving(fig, name + ".mp4", 100):
    for i in range(100):
        print("_Frame: " + str(i), end = "\r")
        fig.suptitle(r'$\rho$ = ' + f'{0.01*i:0.2f}', fontsize=16)
        new_data = g_cor(0.01*i)
        surf.remove()
        surf = ax.plot_surface(x1,x2,new_data, cmap = "gist_ncar", linewidth=0, antialiased=False, rstride=1, cstride=1)
        plt.draw()
        movie.grab_frame()

new_data = g_cor(rho=rho)
surf.remove()
surf = ax.plot_surface(x1,x2,new_data, cmap = "gist_ncar", linewidth=0, antialiased=False, rstride=1, cstride=1)
fig.suptitle(r'$\rho = \frac{2}{3}$', fontsize=16)
plt.draw()
plt.savefig("Salami/Gauss.png", dpi = 300)
plt.show()
print("\nDone      ")